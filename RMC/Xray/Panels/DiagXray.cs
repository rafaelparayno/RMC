...
{
    public partial class DiagXray : Form
    {
        ...
        private void loadCbType()
        {
            ...
            cbLabType.Items.AddRange(cbItems.ToArray());
        }

        private async void refreshDataInCbLab(int labtype)
        {

            List<xraymodel> xraymodels = await xrayControllers.getLabModel(labtype);


            cbLab.DataSource = xraymodels;
        ...
        {
            xmod = (xraymodel)cbLab.SelectedItem;

            if (xmod.autodocsid == 0 && cbLabType.SelectedIndex != -1)
            {
            ...
            else
            {
                ...
                getDisplayAutomated(xmod.autodocsid);
                displayParams(xmod.autodocsid);
            }
        }
